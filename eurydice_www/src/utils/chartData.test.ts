...
describe("chartData", () => {
  ...
  describe("categorical distributions", () => {
    const mixedDistributions: [string, ScalarDistribution][] = [
      ...
      [
        "defend",
        scalarDistribution([[0, 1]], { kind: "categorical", labels: ["HIT"] }),
      ],
      [
        "weather",
        scalarDistribution([[0, 1]], { kind: "categorical", labels: ["SUN"] }),
      ],
    ];
    ...
    it("deduplicates the same observed symbol across local dictionaries", () => {
      expect(
        categoricalOutcomes(mixedDistributions).map(({ label }) => label),
