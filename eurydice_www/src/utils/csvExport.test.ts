...
describe("csvExport", () => {
  ...
  describe("generateSpreadsheetCSV", () => {
    ...
    it("uses explicit tuple field names as CSV columns", () => {
      ...
      expect(result).toBe(
        ["round", "Attacker losses,Defender losses,Probability", "1,2,1"].join(
          "\n",
        ),
      );
    });
  ...
  describe("generateAnyDiceFormatCSV", () => {
    ...
    it("omits categorical outputs", () => {
      const categorical: NamedDistribution = [
        "attack",
        scalarDistribution(
          [
            [0, 0.25],
  ...
  describe("escapeCSVField", () => {
    ...
    it("should handle complex cases with multiple special characters", () => {
      expect(escapeCSVField('output "test", value')).toBe(
        '"output ""test"", value"',
      );
    });
  });
  ...
  describe("CSV generation with special characters in names", () => {
    ...
    it("should handle special characters in generateSpreadsheetCSV", () => {
      ...
      expect(lines[1]).toBe(
        'Outcome,"output ""with quotes""","output, with comma"',
      );
    });
    ...
    it("should handle special characters in generateAnyDiceFormatCSV", () => {
      ...
      expect(lines[0]).toMatch(
        /^"output ""with quotes""",\d+\.?\d*,\d+\.?\d*,1,2$/,
      );

      // Find the second distribution header
      const secondDistLine = lines.find((line) =>
        line.includes("output, with comma"),
      );
      expect(secondDistLine).toMatch(
        /^"output, with comma",\d+\.?\d*,\d+\.?\d*,1,2$/,
      );
    });
  });
  ...
  describe("tuple export", () => {
    ...
    it("places tuple blocks after scalar blocks", () => {
      ...
      expect(result.indexOf("Numeric outcomes")).toBeLessThan(
        result.indexOf("joint"),
      );
    });
  });
});
