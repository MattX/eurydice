...
fn distribution_layout(distribution: &[(i32, f64)], max_width: u16) -> (usize, usize) {
    const PROBABILITY_AND_SEPARATOR_WIDTH: usize = 12;

    let label_width = distribution
        .iter()
        .map(|(outcome, _prob)| outcome.to_string().chars().count())
        .max()
        .unwrap_or_default();
    let max_bar_width =
...
mod tests {
    ...
    fn label_column_uses_widest_label_width() {
        let distribution = [(1, 0.5), (-300, 0.5)];

        assert_eq!(distribution_layout(&distribution, 80), (4, 64));
    }

    #[test]
    fn narrow_terminal_has_zero_width_bar() {
        let distribution = [(-300, 1.0)];

        assert_eq!(distribution_layout(&distribution, 10), (4, 0));
    }
}
